...
final class CommandParserTests: XCTestCase {
    ...
    func testInferPortsFromCustomNamedPortFlags() {
        let tokens = [
            "node",
            "server.js",
            "--api-port",
            "8080",
            "--ui-port=localhost:3000",
            "--metrics-port",
            "127.0.0.1:9090"
        ]
        let ports = CommandParser.inferPorts(from: tokens)

        XCTAssertEqual(ports, [3000, 8080, 9090])
    }
    ...
    func testInferPortsFromDjangoRunserverPortToken() {
    ...
    func testInferPortsFromPythonHTTPServerPortToken() {
        let tokens = ["python3", "-m", "http.server", "8080"]
        let ports = CommandParser.inferPorts(from: tokens)
...
extension CommandParserTests {
    ...
    func testFastAPICommandIsClassifiedAsPythonBackend() {
        let tokens = ["fastapi", "dev", "main.py", "--host", "0.0.0.0", "--port", "8004"]
        let context = CommandParser.makeContext(executable: tokens[0], tokens: tokens, workingDirectory: "/Users/test/app")
        let descriptor = CommandParser.descriptor(from: context)
        let ports = CommandParser.inferPorts(from: tokens)

        XCTAssertEqual(descriptor.displayName, "FastAPI")
    ...
    func testGunicornCommandIsClassified() {
        let tokens = ["gunicorn", "app.main:app", "--workers", "2", "--bind", "0.0.0.0:8002"]
        let context = CommandParser.makeContext(executable: tokens[0], tokens: tokens, workingDirectory: "/Users/test/app")
        let descriptor = CommandParser.descriptor(from: context)
        let ports = CommandParser.inferPorts(from: tokens)

        XCTAssertEqual(descriptor.displayName, "Gunicorn")
        XCTAssertEqual(descriptor.runtime, "Python")
        XCTAssertEqual(descriptor.category, .backend)
        XCTAssertEqual(descriptor.portHints, [8000])
    ...
    func testPumaCommandIsDetectedAsRubyBackend() {
        let tokens = ["bundle", "exec", "puma", "-p", "3004"]
        let context = CommandParser.makeContext(executable: tokens[0], tokens: tokens, workingDirectory: nil)
        let descriptor = CommandParser.descriptor(from: context)
        let ports = CommandParser.inferPorts(from: tokens)

        XCTAssertEqual(descriptor.displayName, "Puma")
        XCTAssertEqual(descriptor.runtime, "Ruby")
        XCTAssertEqual(descriptor.category, .backend)
        XCTAssertEqual(descriptor.portHints, [9292])
        XCTAssertEqual(ports, [3004])
    }

    func testRackupCommandIsDetectedAsRubyBackend() {
        let tokens = ["bundle", "exec", "rackup", "-p", "9293"]
        let context = CommandParser.makeContext(executable: tokens[0], tokens: tokens, workingDirectory: nil)
        let descriptor = CommandParser.descriptor(from: context)
        let ports = CommandParser.inferPorts(from: tokens)

        XCTAssertEqual(descriptor.displayName, "Rackup")
        XCTAssertEqual(descriptor.runtime, "Ruby")
        XCTAssertEqual(descriptor.category, .backend)
        XCTAssertEqual(descriptor.portHints, [9292])
